...
def initialize_qscore_list(length):
    '''intilizes empty list for storing running qscore totals. List length is
    based on length of qscore lines from file'''
    qscores_list =  []
...
def read_file(inp):
    '''reads the input file, capturing qscore lines and adding qscore values
    for each position to a running total for that position'''
    ln = 0
...
def plotting(means, out, record_length):
    '''Outputs the mean qscores for each position for the entire file
    in a bar graph'''
